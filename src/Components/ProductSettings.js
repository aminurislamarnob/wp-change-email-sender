...
const ProductSettings = () => {
	...
	useEffect(() => {
        ...
        const fetchSettings = async () => {
            try {
				...
				if( response.wp_change_email_sender_product_per_page ){
					setProductPerPage(response.wp_change_email_sender_product_per_page);
				}
	...
	const handleSubmit = async ( event ) => {
		...
		setIsLoading( true );
		try {
			const { wp_change_email_sender_product_per_page } = await apiFetch( {
				path: '/wp-change-email-sender/v1/settings',
				method: 'POST',
				data: {
					wp_change_email_sender_product_per_page: productPerPage,
				},
			} );
			setProductPerPage( wp_change_email_sender_product_per_page );
			setMessage(
		...
		<div>
			...
			<form onSubmit={ handleSubmit }>
				<Card>
					<CardBody>
						<TextControl
							label={ __( 'Product per page', 'wp-change-email-sender' ) }
